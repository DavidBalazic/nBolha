...
public protocol NetworkingError: Error {
    ...
    func fieldErrors(for key: String) -> [String]
...
extension NetworkingError {
    ...
    public func fieldErrors(for key: String) -> [String] {
        return userInfo?[key] as? [String] ?? []
    }
    
    public func firstOccurringError() -> String? {
        if let userInfo = userInfo as? [String: [String]],
           let error = userInfo.first?.value.first {
            return error
        }
        if let userInfo = userInfo as? [String: String],
           let error = userInfo.first?.value {
            return error
        }
        if let userInfo = userInfo, let error = userInfo["error"] as? String {
            return error
        }
        if let userInfo = userInfo, let error = (userInfo["errors"] as? [String: String])?.values.first {
            return error
        }
